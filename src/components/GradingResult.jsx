...
import { CheckCircle2, Trophy, RefreshCcw, ArrowRight, CheckCircle2 as OIcon, XCircle, BookOpen, AlertTriangle, Trash2 } from 'lucide-react';
    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-bottom-8 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* 내 답안 (드로잉 모드) */}
                {inputMode === 'draw' && userAnswer && (
                     <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-xl">
                         <h3 className="text-slate-900 font-bold mb-4 flex items-center gap-2">
                            <CheckCircle2 size={20} className="text-blue-600" /> 나의 답안
                        </h3>
                        <div className="p-2 bg-slate-50 rounded-xl border border-slate-100 overflow-hidden">
                            <img src={userAnswer} alt="User's drawing" className="w-full h-auto object-contain" />
                        </div>
                    </div>
                )}
                ...
                {/* 모범 답안 */}
                <div className={`space-y-4 ${inputMode === 'draw' && !userAnswer ? 'col-span-2' : ''}`}>
                    <div className="bg-emerald-900/30 rounded-2xl p-6 border border-emerald-500/30 relative group/answer">
                        ...
                        {/* 문제 지문 이미지 */}
                        {currentProblem.questionImageUrl && !isEditMode && (
                            <div className="relative mb-4">
                                <p className="text-xs text-emerald-300 font-bold mb-1">[문제 지문]</p>
                                <div className="rounded-lg overflow-hidden border border-emerald-500/30">
                                    <img src={currentProblem.questionImageUrl} alt="Question content" className="w-full object-contain" />
                                </div>
                                <button onClick={() => handleDeleteImage(currentProblem.id, 'questionImageUrl')} className="absolute top-0 right-0 p-1.5 bg-red-600/80 hover:bg-red-500 text-white rounded-bl-lg transition-all opacity-0 group-hover/answer:opacity-100">
                                    <Trash2 size={16} />
                                </button>
                            </div>
                        )}
                        ...
                        {currentProblem.answerImageUrl && !isEditMode && (
                            <div className="relative">
                                <p className="text-xs text-emerald-300 font-bold mb-1">[모범 답안]</p>
                                <div className="rounded-lg overflow-hidden border border-emerald-500/30">
                                    <img src={currentProblem.answerImageUrl} alt="Answer Key" className="w-full object-contain" />
                                </div>
                                <button onClick={() => handleDeleteImage(currentProblem.id, 'answerImageUrl')} className="absolute top-0 right-0 p-1.5 bg-red-600/80 hover:bg-red-500 text-white rounded-bl-lg transition-all opacity-0 group-hover/answer:opacity-100">
                                    <Trash2 size={16} />
                                </button>
                            </div>
                        )}
