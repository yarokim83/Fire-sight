...
const ProblemCard = ({ state, actions }) => {
    ...
    const { setIsEditMode, setShowMemo, setMemoText, handleSaveMemo, setCurrentProblem, handleSaveEdit, handleImageUpload, handleDelete, handleDeleteImage } = actions;
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl relative overflow-hidden group">
            ...
            {currentProblem.questionImageUrl && !isEditMode && (
                <div className="mb-6 rounded-xl overflow-hidden border border-slate-700 bg-black/50 relative z-10">
                    <img src={currentProblem.questionImageUrl} alt="Problem Reference" className="w-full max-h-80 object-contain mx-auto" />
                </div>
            )}
                    ...
                    <div className="space-y-3 animate-in fade-in">
                        ...
                        <div className="flex items-center gap-2">
                            <label className="flex items-center gap-2 px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded cursor-pointer border border-slate-700 text-sm transition-colors">
                                <ImageIcon size={16} />
                                {currentProblem.questionImageUrl ? "지문 변경" : "지문 추가"}
                                <input type="file" accept="image/*" className="hidden" onChange={(e) => handleImageUpload(e, 'questionImageUrl')} />
                            </label>
                            {currentProblem.questionImageUrl && (
                                <button onClick={() => handleDeleteImage(currentProblem.id, 'questionImageUrl')} className="p-2 text-red-400 hover:bg-red-500/10 rounded">
                                    <Trash2 size={16} />
                                </button>
                            )}
                            <label className="flex items-center gap-2 px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded cursor-pointer border border-slate-700 text-sm transition-colors">
                                <ImageIcon size={16} />
                                {currentProblem.answerImageUrl ? "답안 변경" : "답안 추가"}
                                <input type="file" accept="image/*" className="hidden" onChange={(e) => handleImageUpload(e, 'answerImageUrl')} />
                            </label>
                            {currentProblem.answerImageUrl && (
                                <button onClick={() => handleDeleteImage(currentProblem.id, 'answerImageUrl')} className="p-2 text-red-400 hover:bg-red-500/10 rounded">
                                    <Trash2 size={16} />
                                </button>
                            )}
                        </div>
