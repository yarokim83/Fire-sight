...
export default function Sidebar({ 
    ...
    isAuthenticated, handleLogout, isMobile 
}) {
    ...
    return (
        <>
            {/* 모바일 화면에서 열렸을 때 뒷배경 어둡게 차단하는 Backdrop */}
            {isMobile && !isCollapsed && (
                <div 
                    onClick={() => setIsCollapsed(true)} 
                    className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[90] animate-in fade-in duration-300 cursor-pointer"
                />
            )}
            
            <aside className={`
                ${isMobile 
                    ? `fixed left-0 top-0 h-screen w-64 z-[100] transition-transform duration-300 ${isCollapsed ? 'translate-x-[-100%]' : 'translate-x-0 shadow-2xl'}`
                    : `relative z-50 ${isCollapsed ? 'w-20' : 'w-64'} transition-all duration-300 ease-in-out`
                }
            ...
            `}>
                ...
                <div className={`h-24 flex items-center ${isCollapsed && !isMobile ? 'justify-center' : 'px-6'} transition-all`}>
                        ...
                        return (
                            <React.Fragment key={item.mode}>
                                ...
                                <button
                                    ...
                                    className={`
                                        w-full flex items-center gap-3.5 px-3.5 py-3 rounded-xl transition-all duration-200 group relative cursor-pointer
                                        ${isActive 
                ...
                {/* 3. 🔴 사용자님이 가장 선호하신 '핸들형 토글' (디자인 고도화, 모바일 제외) */}
                {!isMobile && (
                    <button
                        onClick={() => setIsCollapsed(!isCollapsed)}
                        className="absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-14 bg-white/10 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center text-white/40 hover:text-white hover:bg-white/20 transition-all shadow-xl z-[60] group cursor-pointer"
                    >
                ...
                <div className="p-4 border-t border-white/5 bg-white/[0.02]">
                    <div className={`flex items-center justify-between ${isCollapsed && !isMobile ? 'justify-center' : 'px-2'}`}>
                        <div className="flex items-center gap-3">
                            <div className={`w-2 h-2 rounded-full ${isAuthenticated ? 'bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.5)]' : 'bg-white/10'}`} />
                            {(!isCollapsed || isMobile) && <span className="text-[10px] font-bold text-white/20 uppercase tracking-widest">{isAuthenticated ? 'Live' : 'Offline'}</span>}
                        </div>
                        {(!isCollapsed || isMobile) && isAuthenticated && (
                            <button onClick={handleLogout} className="text-[9px] font-bold text-white/30 hover:text-rose-400 transition-colors cursor-pointer">EXIT</button>
                        )}
                    </div>
                </div>
            </aside>
